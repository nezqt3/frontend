            ...
            <div className="orders">
                {orders.map((elem, index) => {
                    ...
                    return (<div className="order" key={index}>
                        <div className="order-header" onClick={() => toggleOrder(index)}>
                            <img src={arrowDown} alt="arrow-down" className={`arrow ${isOpen ? "rotated" : ""}`}/>
                            <p>{index + 1}.</p>
                            <p>{elem.nameOfOrder}</p>
                            <img src={lineOrder} alt="line-order" className="line-order"/>
                            <p>{elem.cost}</p>
                        </div>
                        {isOpen && (
                        <div className="upper-block">
                            <p>Дата создания - <b>{elem.dateOfCreate}</b></p>
                            <p>Адрес - {elem.address}</p>
                            <p>Размер - <b>{elem.size}</b></p>
                            <p>Количество - <b>{elem.count} шт</b></p>
                            <b>Итог - 16800 ₽</b>
                        </div>
                        )}
                    </div>)
